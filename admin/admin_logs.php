...
<body>
        ...
        <div class="log-content">
            ...
            <div id="system-tab" class="log-section">
                ...
                <div class="log-viewer">
                    ...
                    <div class="log-line">
                        <span class="log-timestamp">[<?php echo date('Y-m-d H:i:s'); ?>]</span>
                        <span class="log-message">Base de données: <?php echo DB_NAME; ?></span>
                    </div>
                    <div class="log-line">
                        <span class="log-timestamp">[<?php echo date('Y-m-d H:i:s'); ?>]</span>
                        <span class="log-message">Utilisateurs connectés: <?php echo count($logContents['activity'] ? explode("\n", $logContents['activity']) : []); ?></span>
                    </div>
                    <div class="log-line">
                        <span class="log-timestamp">[<?php echo date('Y-m-d H:i:s'); ?>]</span>
    ...
    function clearCurrentLog() {
        ...
        fetch('admin_api.php?action=clear_log', {
            method: 'POST',
            headers: {
    ...
    function clearAllLogs() {
        if (!confirm('Effacer tous les journaux ? Cette action est irréversible.')) {
            return;
        }

        fetch('admin_api.php?action=clear_all_logs')
        .then(response => response.json())
        .then(result => {
            alert(result.message);
            if (result.success) {
    ...
    setInterval(() => {
        if (!document.hidden) {
            fetch('admin_api.php?action=check_log_updates')
            .then(response => response.json())
            .then(result => {
                if (result.updated) {
                    refreshLogs();
